...
def getFreeThrows(gameId):
    ...
    filtered = []
    for a in pbp:
        if "0:00" != a[6].encode("utf-8"):
            continue
        if a[9] != None:
            if "PTS" in a[9].encode("utf-8"):
                filtered.append(a[9])
        if a[7] != None:
            if "PTS" in a[7].encode("utf-8"):
                filtered.append(a[7])

    return len(filtered) >= 3 and len(pbp) != 0
...
def finalThing(season):
    ...
    for a in range(0, 1250):
        print "season %d game %d" % (season, a)
...
#for a in range(15):
#    print finalThing(a)
#print getFreeThrows("0021600550")
_get = requests.get('http://stats.nba.com/stats/playbyplay', params={"GameID":"0021600550","StartPeriod":1,"EndPeriod":10}, headers=headers)
_get.raise_for_status()
pbp = _get.json()['resultSets'][0]['rowSet']
